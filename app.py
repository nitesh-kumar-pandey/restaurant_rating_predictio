...
file = open("rating_prediction.pkl", "rb")
...
def predictor():
    if request.method == "POST":
        ...
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO restaurant (online_order, book_table, votes, approx_cost, location, rest_type, cuisines)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            data['online_order'], data['book_table'], data['votes'], data['approx_cost'], location, ','.join(rest_type), ','.join(cuisines)
        ))
        conn.commit()
        conn.close()
